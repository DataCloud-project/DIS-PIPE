...
ShrinkSymbolic::ShrinkSymbolic(const Options &opts)
  : ShrinkStrategy(opts),
    group_by_h(opts.get<bool>("group_by_h")) {
}

ShrinkSymbolic::~ShrinkSymbolic() {
}

string ShrinkSymbolic::name() const {
  return "symbolic";
}

void ShrinkSymbolic::dump_strategy_specific_options() const {
  cout << "Group by h: " << (group_by_h ? "yes" : "no") << endl;
}

bool ShrinkSymbolic::reduce_labels_before_shrinking() const {
  return true;
}

void ShrinkSymbolic::shrink(
			    SymbolicAbstraction &abs, int target, bool force) {
...
void ShrinkSymbolic::compute_abstraction(SymbolicAbstraction &abs,
					 int target_size,
					 EquivalenceRelation &equivalence_relation) {
...
static ShrinkStrategy *_parse(OptionParser &parser) {
  ...
  if (!parser.dry_run())
    return new ShrinkSymbolic(opts);
  else
    return 0;
}

static Plugin<ShrinkStrategy> _plugin("shrink_symbolic", _parse);
