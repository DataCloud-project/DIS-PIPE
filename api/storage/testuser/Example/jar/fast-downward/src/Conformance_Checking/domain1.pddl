...
:effect (and (allowed) (not (token p_6)) (token sink) (not (tracePointer ev9)) (tracePointer evEND))
)

(:action moveInTheModel#paycompensation
:precondition (token p_6)
:effect (and (not (allowed)) (not (token p_6)) (token sink) (increase (total-cost) 1)
)
)
