...
VariableOrderFinder::VariableOrderFinder(MergeStrategy merge_strategy_, bool is_first, const vector <int> & remaining_vars_)
  : merge_strategy(merge_strategy_), remaining_vars(remaining_vars_) {
    int var_count = g_variable_domain.size();
